...
eat_or_think(void *arg)
{
    ...
    for (;;){
        ...
        sem_post(&waiter);
        printf("%sPhilosopher %d leaves the room" EOL, colors[n], n);
    }
    pthread_exit(NULL);
}
...
main(int argc, const char *argv[])
{
    ...
    sem_init(&waiter, SHARED_THREADS, NUM_PHILO - 1);
    for (i = 0; i < NUM_PHILO; i++){
